...
`openspec init` while preserving this repository's `openspec/` layout and verify the
...
- AND it runs `openspec init` with the shared `openspec/` layout retained
...
- [x] Documentation states the command-skill prerequisites: Node.js 26 or later,
  ...
  `openspec init` while retaining `openspec/` paths.
